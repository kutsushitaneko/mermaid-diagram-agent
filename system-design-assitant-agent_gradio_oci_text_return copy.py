...
def process_user_message_with_agent(user_message):
    ...
    try:   
        ...
        # ダイアグラム生成タスクかどうかを判定
        is_diagram_task = any(keyword in user_message.lower() for keyword in [
            "図", "ダイアグラム", "diagram", "chart", "グラフ", "設計図", "フローチャート", 
            "er図", "クラス図", "シーケンス図", "状態遷移図", "ネットワーク図", "組織図",
            "mermaid", "可視化", "描画", "表示"
        ])
        
        # ダイアグラム生成タスクの場合のみファイル検索を実行
        if is_diagram_task:
            ...
            # スクリプトファイルのパスも検索
            mmd_pattern = r'output[/\\]mermaid_diagram_\d{8}_\d{6}\.mmd'
            mmd_match = re.search(mmd_pattern, result_str)
            
            if png_match:
                image_file = png_match.group(0)
                script_file = mmd_match.group(0) if mmd_match else ""
                ...
                # ダイアグラムタスクだがファイルが見つからない場合
                return (
                    response_text,
                    "警告: ダイアグラム生成が要求されましたが、画像ファイルが見つかりませんでした。",
                    "",
                    "",
                    None,
                    ""
